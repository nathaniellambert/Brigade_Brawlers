...
back1 = pygame.image.load('boxing-ring-background.jpg')
...
# controls screen
def controls_screen():
    icon_names = ['arrow-keys.png', 'wasd.png', 'm-key.png', 'f-key.png']
    icons = []
    for icon in icon_names:
        ctrl_img = pygame.image.load(icon)
        icons.append(ctrl_img)
    run = True
    while run:
        ...
        p1_cntrls = font.render('PLAYER 1 CONTROLS:', True, GOLD)
        p2_contrls = font.render('PLAYER 2 CONTROLS:', True, GOLD)
        # MOVEMENT ICONS
        screen.blit(p1_cntrls, (50, 50))
        screen.blit(font.render('MOVEMENT', True, AMBER), (120, HEIGHT/2+50))
        screen.blit(p2_contrls, (600, 50))
        screen.blit(font.render('MOVEMENT', True, AMBER), (680, HEIGHT/2+40))
        # PUNCH CONTROLS
        screen.blit(icons[2], (50+100, 400))
        screen.blit(icons[3], (600+100, 400))
        screen.blit(font.render('PUNCH', True, AMBER), (120+15, 3*HEIGHT/4+20))
        screen.blit(font.render('PUNCH', True, AMBER), (680+5+3, 3*HEIGHT/4+20))

        screen.blit(font.render('[C] : CONINTUE', True, GOLD), (WIDTH/2-80, HEIGHT-80))
...
def main_window(dimensions, caption):
    ...
    # back1 = pygame.image.load('/Users/x10326/Desktop/Academics/AY19-1/CY300/Project/boxing-ring-background.jpg')
    screen.blit(back1, (0,0))
    pygame.display.update()
...
'''def jump(fighter):
    if fighter.jumping:
        fighter.jump_offset += 1
        if fighter.jump_offset >= JUMP_HEIGHT:
            fighter.jumping = False
    elif fighter.jump_offset > 0 and fighter.jumping == False:
        fighter.jump_offset -= 1'''
...
def play_game():
    start_screen()
    player1_icon_num = choose_player(1)
    player2_icon_num = choose_player(2)
    controls_screen()
    # make display appear
    display = main_window((WIDTH,HEIGHT),"PE117")

    # load fighters
    player1 = Fighter(50,50,player1_icon_num,0)
    player2 = Fighter(50,100,player2_icon_num,0)
    player1.centerx = 0
    player1.centery = 0
    ...
    stand1 = [pygame.image.load('P1T30.png'),pygame.image.load('P1T31.png'),pygame.image.load('P1T32.png')]
    stand2 = [pygame.image.load('P2T30.png'),pygame.image.load('P2T31.png'),pygame.image.load('P2T32.png')]
    punch1 = [pygame.image.load('plebepunch2.png')]
    punch2 = [pygame.image.load('reversepunch.png')]
    retreat1 = [pygame.image.load('retreat0.png')]
    retreat2 = [pygame.image.load('reverseretreat.png')]
